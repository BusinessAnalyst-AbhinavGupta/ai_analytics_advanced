...
**Test status:** 193/193 standalone tests pass (all `tests/` modules except legacy `test_ui_and_db`, which hangs only because it targets an unreachable Neo4j). Live Metabase tests: 3/3; live DT funnel drive: 3/3 junior analyses (conversion, error-rate, funnel volumes) with MD review files.
